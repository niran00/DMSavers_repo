...
var mylat = getParameterByName('lat');
var mylng = getParameterByName('lng');

/*
if (mylat || mylng) {
            // If centre point doesn't exist yet set it
            if (self.centrePoint === '') {
                self.centrePoint = new google.maps.LatLng(mylat, mylng);
                self.defaultZoom = parseInt(mapObject.options.defaultzoom, 10);
            }
			
			showGeoMarker(mylat, mylng);	
                ...
                if (geocoder) {
                    geocoder.geocode({'address': address}, function (results, status) {
                        if (status == google.maps.GeocoderStatus.OK) {
                            ...
                            self.geocodedLocation({lat:mylat, lng:mylng});
